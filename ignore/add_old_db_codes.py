...
def add_composer_search(pml):
    pml["composer_search"] = pml["composer"].str.lower().str.replace(
        r"[^a-zA-Z]", "", regex=True
    )
    pml["composer_last"] = pml["composer"].str.lower().str.split(" ").str[-1]
    pml["composer_last"] = pml["composer_last"].str.replace(r"[^a-zA-Z]", "", regex=True)
    pml["composer_no_hyphen"] = pml["composer"].str.lower().str.split("-").str[-1]
    pml["composer_no_hyphen"] = pml["composer_no_hyphen"].str.replace(
        r"[^a-zA-Z]", "", regex=True
    )
...
def update_db(code, n, entry_number, lock):
    try:
        lock.acquire()
        conn = sqlite3.connect("uil_old.db")
        c = conn.cursor()
...
def main():
    ...
    uil_new_db = sqlite3.connect("uil.db")
    uil_new_df = pd.read_sql("SELECT * FROM results", uil_new_db)
    pml_new = pd.read_sql("SELECT * FROM pml", uil_new_db)
    uil_new_db.close()

    # get rid of old_df rows where entry_number is in new_df
    uil_data = uil_data[~uil_data["entry_number"].isin(uil_new_df["entry_number"])]
    ...
    conn.execute(
        "CREATE TABLE IF NOT EXISTS unique_entries (code INTEGER PRIMARY KEY, title TEXT, composer TEXT, composer_last TEXT, composer_no_hyphen TEXT, event TEXT)"
    )

    # replace pml with pml_new
    conn.execute("DROP TABLE pml")
    pml_new.to_sql("pml", conn, index=False)

    pml = pd.read_sql("SELECT * FROM pml", conn)
    pml = add_composer_search(pml)
