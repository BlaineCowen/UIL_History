...
prefs = {
    "download.default_directory": r"F:\Documents\projects\UIL_RESULTS_REWRITE\csv_downloads\\"
}
...
def get_uil_results():
    ...
    select_year = Select(
        driver.find_element(
            By.XPATH,
            '//*[@id="content-wrapper-div"]/div/div/div/div/div/div[1]/div/div/form/div[1]/select',
        )
    )
    ...
    for i in range(len(year_options)):
        ...
        region = Select(
            driver.find_element(
                By.XPATH,
                '//*[@id="content-wrapper-div"]/div/div/div/div/div/div[1]/div/div/form/div[2]/select',
            )
        )
        ...
        for j in range(len(region_options) - 2):
            ...
            event = Select(
                driver.find_element(
                    By.XPATH,
                    '//*[@id="content-wrapper-div"]/div/div/div/div/div/div[1]/div/div/form/div[3]/select',
                )
            )
            event_options = event.options
            for c in range(len(event_options) - 1):
                year = Select(
                    driver.find_element(
                        By.XPATH,
                        '//*[@id="content-wrapper-div"]/div/div/div/div/div/div[1]/div/div/form/div[1]/select',
                ...
                event.select_by_index(c + 1)
                time.sleep(0.5)
                try:
                    contest = Select(
                        driver.find_element(
                            By.XPATH,
                            '//*[@id="content-wrapper-div"]/div/div/div/div/div/div[1]/div/div/form[2]/div/select',
                        )
                    )
                except exceptions.NoSuchElementException:
                ...
                for k in range(len(contest_options) - 1):
                    ...
                    try:
                        results = driver.find_element(
                            By.XPATH,
                            '//*[@id="content-wrapper-div"]/div/div/div/div/div/div[1]/div/div/form[2]/button[1]/b',
                        ).click()
                    except:
